...
function loadClinResearchEnv(): Record<string, string> {
  const path = join(process.env.XDG_CONFIG_HOME ?? join(homedir(), ".config"), "opencode", "clinresearch.env")
  ...
  try {
    ...
    getLogger().warn("failed to load ClinResearch model environment", error)
    return {}
  }
}
